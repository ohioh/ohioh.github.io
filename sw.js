...
self.addEventListener('fetch', event => {
  console.log('[OHIOH]: Fetch event for ', event.request.url);
  event.respondWith(
    caches.match(event.request)
    .then(response => {
      if (response) {
        console.log('[OHIOH]: Found ', event.request.url, ' in cache');
        return response;
      }
      console.log('[OHIOH]: Network request for ', event.request.url);
...
self.addEventListener('notificationclose', event => {
  ...
  console.log('[OHIOH]: Closed notification: ' + primaryKey);
});
